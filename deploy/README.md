...
## Онлайн-запись и WhatsApp

Клиенты записываются по ссылке `https://<домен>/book/<slug клиники>` (ссылка и
QR-код есть в CRM: Управление → Онлайн-запись). Коды подтверждения и
уведомления уходят клиенту **с WhatsApp-номера самой клиники** через наш шлюз
`wa-gateway` (контейнер в том же compose, наружу не публикуется).

В `.env` на сервере добавьте:

```
PUBLIC_BASE_URL=https://<домен>          # без завершающего слэша
MESSENGER_PROVIDER=baileys               # noop - выключить отправку
WA_GATEWAY_TOKEN=<openssl rand -hex 24>  # общий секрет backend ↔ wa-gateway
TELEGRAM_BOT_TOKEN=                      # необязательно, токен от @BotFather
```

Подключение номера: владелец клиники заходит в CRM → Управление →
Онлайн-запись → «Подключить WhatsApp» и сканирует QR телефоном клиники
(WhatsApp → Связанные устройства → Привязать устройство). Сессии хранятся в
томе `wa_auth` и переживают перезапуски; бэкап кладёт их в `wa-*.tgz`.

Если позже перейдёте на Green API: `MESSENGER_PROVIDER=greenapi`, а
`idInstance`/`apiTokenInstance` каждая клиника вводит у себя в настройках
онлайн-записи.
