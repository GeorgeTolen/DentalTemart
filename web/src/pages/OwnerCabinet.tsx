...
export default function OwnerCabinet() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">{t("Личный кабинет")}</h1>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <SummaryCard label={t("Приёмов сегодня")} value={dashboard?.today_count ?? 0} />
        <SummaryCard label={t("Приёмов за неделю")} value={dashboard?.week_count ?? 0} />
        <SummaryCard label={t("Пациентов")} value={stats?.total_patients ?? 0} />
        <SummaryCard label={t("Врачей")} value={stats?.total_doctors ?? 0} />
      </div>
